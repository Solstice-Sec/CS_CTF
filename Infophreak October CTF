...
The flag is case insensitive, so do not worry about upper/lower case.
This one is just about knowing your history as it comes to cybersecurity. Equation Group is suspected to be related to the NSA through their Tailored Access Operations unit. EquationDrug is one of the first bits of malware discovered and classified by Kaspersky. 
CTF{[TAO][EquationDrug][6bcd591540dce8e0cef7b2dc6a378a10d79f94c3217bca5f05db3c24c2036340]}
...
Easy 🟢
Description
Continuing your analysis on the 2014 incident described in part 1, let's dive deeper into the catalyst for detection. All of the below questions refer the known malware variant that Kaspersky initially discovered before identifying the unknown Equation Group malware.
Answer these questions to build the flag:
...
Most OSINT tasks such as this can be automated by using AI. I used Claude in this case, but it is worth noting that AI still has the hallucination issue, meaning you should always double check the AI’s methodologies for collecting information as well as double checking the information itself. CTF{[steam.exe][hvgpj][Yasir Shah]}
...
Flag format CTF{[Q1][Q2][Q3]}.
The flag is case insensitive, so do not worry about upper/lower case.
After much arguing with Grok, Claude and ChatGPT, I was able to get the image referenced in Q3. I learned from this task that Grok really doesn’t have good OCR capabilities. It kept reading the ID wrong or reading the ID above the one needed. The link was https://web.archive.org/web/20180719085116im_/http://i.imgur.com/2uQWr5E.png

CTF{[SmokeLdr][smoke@exploit.im][8774C90BC844D839246FBCFFA31B75F4EC2B64C6]}
...
IP Halloween 2025 - Gustavus Amrita’s Nightmare 1, 2, 3, 4:
Difficulty
Medium 🟡
Description
...
I used the following URL to check this.
https://www.joesandbox.com/analysis/1623555/0/html
What is the name of the MSI file used in the attack?
CTF{3aw.msi}
What malicious domain is used in the attack?
CTF{bestiamos.com}
What URL-shortening service was used in this attack?
CTF{short.gy}
What series of keyboard combinations was the victim social engineered into making? Format: ALT+A;CTRL+B;SHIFT
...
CTF{WIN+R;CTRL+V;ENTER}

IP Halloween 2025 - Theory of Deduction:
Difficulty
Hard 🔴
Description
Instructions file are inside the zip
Each puzzle hides a single letter, number, or symbol.
Piece them together, and they reveal the key to the sealed treasure.
...
“From where you stand, amid glass and steel,
A pale sky gleams, and daylight reveals.
White upon blue, a name comes into view,
Behind the shine, the first letter of my word guides you through.
Do not step forward, not an inch away,
Just turn around and look close — the clue won’t stray.”


2nd puzzle 


“A giant wheel frames the sky ahead,
Its circle watches, though nothing is said.
Think of the name this great eye guards —
And keep the first round among its parts.”


3rd puzzle


"Stand firm, take not a single step,
Rotate your view — let your eyes adept.
A house of skill and craft appears,
Its letters hide a force revered.
Like a racer nearing the final bend,
Taking third as the journey ends."


4th puzzle 


"Stand still where ancient stone holds its throne,
As arches murmur of ages long gone.
Beyond these ruins, a name stands tall —
The Eternal City, revered by all.
Within its name, where all counts begin,
A hollow waits — the first that steps in."


5th puzzle


"Stand firm where arches stretch and climb,
Steel and stone hold back the time.
Look around, yet do not tread,
A hidden link lies just ahead.
Beneath your gaze, it spans unseen,
The quiet mark that joins between."


6th puzzle


“Before you, cliffs rise like ancient guards,
Waves crash with fury, winds sing of bards.
The God of Thunder bends the northern skies,
Lightning arcs where a hidden isle lies.
Though your feet rest on familiar stone,
Beyond the lands where larger isles slumber,
Seek the wild northern waters’ thunder.
The first of this isle begins your plunder.”


7th puzzle 


Among these quiet merchant streets,
where gulls trace secrets on the wind,
stands a sentinel who outlived all others.
It has watched empires trade and tides retreat,
older than the stones you tread,
and the very first to pierce this coast with fire.
Find this eldest of the beacons, untouched by time’s wrath,
...
Beneath the spires of storied halls,
Where rival minds once paced the walls,
A hidden lab stitched silent streams,
Through copper veins and circuit dreams.


A single mind joined name to name,
And lit the spark that changed the game.
Trace the arc where messages unite
The curl that bridges left and right.


9th


Beneath the towers of gold and light,
Two warriors clashed in a fabled fight.
Yet one was struck, the crown slipped from his hand,
Fortune and glory scattered across the land.


A lone sigil waits where heroes fell,
Follow the vanquished to claim the spell.
The first of the name they bear last,
Unlocks the key to complete your task.


All of these have pictures attached that you are supposed to glean information from. I didn’t want to waste time putting each guess into the 7zip password file, so I tried using JohnTheRipper, since the naming is thematically appropriate and probably intended. I tried both John and Hashcat, but they both found nothing. I figured it was probably a technical issue rather than just not having the password, so I created a password protected 7z file and tested both against it, this time with a password list with a known good password. Didn’t work. Ultimately, I wound up using a script to perform the same task.
...
Challenge URL
https://web-ctfs.vercel.app/encoded-message
The provided chatbot only communicated in quotes or on specific key words. I typed “HELP” into it, to which it responded:
...
May 12, 2019 is the date for the quote. When entered:
...
Challenge URL
https://pastebin.com/BvF8vM2Y
Paste contains a file with a single skull emoji. Downloading the file and viewing it in Notepad shows that there are unknown characters as well. These characters are Unicode tags. We can use the following website for looking up each of these characters:
https://r12a.github.io/uniview/
Looking them up, we get a list of variation selectors. We extract the common values from them to end up with ASCII byte values. If we translate them into ASCII resulting in "3D6k<?<JO75DOB5;DO=(m". We finally add 16 decimal to the bytes to return CTF{LOLZ_GET_REKT_M8}.
...
Challenge URL
https://pastebin.com/5Z4Ey6q2
Second verse, same as the first. We grab the hidden characters using the website in the first part of this problem series. The flag is CTF{0K_Y0U_GOT_M3}.
...
Challenge URL
https://pastebin.com/AZJdNnwg
The pattern repeats. Same website. CTF{U_AR3_4_W1Z4RD_HARRY}
IP Halloween 2025 - Bones 4:
Same as above.
CTF{H0LY_GU4CAM0L3_UR_CRACKD}
...
Just one reason, to hack the house
But in the day time the streets was clear
You couldn't find a good phreak anywhere, 'cause
The phreaks come out at night
The phreaks come out at night
The phreaks come out at night
(The phreaks come out)
The phreaks come out at night
The phreaks come out at night
The phreaks come out at night
The phreaks come out at night
(The phreaks come out)
The phreaks come out at night
Now when phreaks get dressed to go out at night
They like to wear leather jackets, CTF{ch4ins&sp1kes}
They wear rips and zippers all in their shirts
Real fire hats or fresh infophreak merch
All kinds of colors runnin' through their hair
And you could just about spot a phreak anywhere
But then again, you could know someone all their life
But might not know they're a phreak unless you see them at night, 'cause
The phreaks come out at night
The phreaks come out at night
The phreaks come out at night
(The phreaks come out)
...
This is a Vigenere cipher. AI systems couldn’t solve it correctly, so I found a website that would solve it for me.You can tell it is Vigenere by the fact that the flag got turned into CVP from CTF. This is the site used: https://www.guballa.de/vigenere-solver
Boys and girls of every age
Wouldn′t you like to see the CTF key today?
...
The CTF’s waiting for the pounce and how you'll
CTF{SCR34M!}
This is Halloween
...
In our town of Halloween
In this town, don't we love it now?
Everyone′s waitin′ for the next surprise
...
I went through a number of tools on Kali Linux to try to work this out, but eventually found a webtool that would print it out. This is the image from the audio.

I almost think it looks like a beholder, but the fingers tell me otherwise.
CTF{OMGth3yAreCOnTroLL1ngMYm1nd}
Reverse Engineering
...
The challenge starts with a zip file containing a 7z file with the flag in a text file and a class file that contains the key to unlock it. I tried to open it in Notepad++, Eclipse and Visual Studio, which all resulted in unclear text such as above. 

I put the file into Claude to clear up the file. It found that it contained a Base64 string that was XORed with a key. Claude wrote a program to reverse this encoding and executed it resulting in the following:
...
This was easily solved by plugging it into Claude. The password was berries1818. The first things he discovers in a forest are berries. The year of the novel’s publication was a natural choice for the 4 digits.
